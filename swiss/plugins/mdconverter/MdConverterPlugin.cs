...
namespace plugins.mdconverter
{
    internal class MdConverterPlugin : Plugin
    {
        ...
        public override async Task RunAsync(string[] args, CancellationToken ct)
        {
            ...
            await writer.WriteLineAsync("<link rel=\"stylesheet\" href=\"https://cdnjs.cloudflare.com/ajax/libs/prism-themes/1.9.0/prism-vsc-dark-plus.min.css\" crossorigin=\"anonymous\" referrerpolicy=\"no-referrer\"/>");
            ...
            await writer.WriteLineAsync("<style>body,pre{font-size:1.1em}:not(pre)>code,code[class*=language-],pre[class*=language-]{font-size:.9em;font-family:\"JetBrains Mono\",monospace!important}ol,p,ul{margin:5px 0}:root{--bc:#fff;--hr:#ddd;--color:#151515;--main:#d63384;--code-bc:#f1f1f1;--blockquote-border:#dfe2e5;--blockquote-bc:#f1f1f1;--blockquote-color:#222}body.dark{--bc:#111;--hr:#333;--color:#eee;--code-bc:#151515;--blockquote-border:#252525;--blockquote-bc:#151515;--blockquote-color:#ddd}*{box-sizing:border-box}body{background-color:var(--bc);font-family:Roboto;line-height:1.5;color:var(--color)}pre{font-family:\"Jetbrains Mono\"!important;tab-size:4;padding:10px;border-radius:10px}:not(pre)>code{background-color:var(--code-bc);padding:2px 5px;border-radius:4px;color:var(--main)}h1{border-bottom:2px solid var(--hr);margin-bottom:.3em}h2{border-bottom:1px solid var(--hr);margin-bottom:.2em}hr{border:none;display:block;background-color:var(--hr);height:2px}ol,ul{padding-inline-start:25px}blockquote{border-left:.25em solid var(--blockquote-border);background-color:var(--blockquote-bc);color:var(--blockquote-color);padding:.5em 1em;margin-left:0}table{border-collapse:collapse;width:100%;margin:15px 0;border:1px solid var(--hr)}th,td{padding:8px 12px;border:1px solid var(--hr);text-align:left}th{background-color:var(--code-bc);font-weight:700}tr:nth-child(even){background-color:rgba(128,128,128,0.05)}</style>");
            ...
            await foreach (var lineExtracted in File.ReadLinesAsync(mdPath, ct))
            {
                ...
                if (line.StartsWith('|'))
                {
                    if (!InTable)
                    {
                        CloseTags(writer);
                        InTable = true;
                        HeaderWritten = false; // FIX: Inizializza a false, non l'abbiamo ancora scritto!
            ...
            // # --------------------- #
            if (convertToPdf) ConvertToPdf(htmlFilePath, keepHtml, ct);
        ...
        private async void ConvertToPdf(string htmlFilePath, bool keepHtml, CancellationToken ct)
        {
            ...
            string argsStr = $"--headless --print-to-pdf=\"{pdfFilePath}\" --no-pdf-header-footer --disable-gpu \"file:///{htmlFilePath.Replace('\\', '/')}\"";
